...
<body>
    <div class="container">
        ...
        <?php if (isset($summaryResults) && !empty($summaryResults)): ?>
            ...
            <!-- Display Name, Total Responses, Total Ratings -->
            <div>
                <p><strong>Name:</strong> <?= esc($facultyName) ?></p> <!-- Assuming faculty name is passed as a variable -->
                <p><strong>Total Number of Responses:</strong> <?= esc($totalResponses) ?></p> <!-- You can calculate this based on the evaluations -->
                <p><strong>Total Ratings:</strong> <?= esc($totalRatings) ?></p> <!-- Total ratings count -->
            </div>
                ...
                <tbody>
                    <?php foreach ($summaryResults as $index => $result): ?>
                        <tr>
                            ...
                            <td><?= esc($result['total_responses']) ?></td>
                            <td><?= esc($result['total_ratings']) ?></td>
            ...
            <div class="card">
                ...
                <?php foreach ($summaryResults as $result): ?>
                    <div class="evaluation">
                        <h4>Evaluation Question: <?= esc($result['question_text']) ?></h4>
                        <p><strong>Tokenized Comments:</strong>
                            <?php
                                $tokenizedComments = !empty($result['tokenized_comment']) ? esc($result['tokenized_comment']) : "No comments available.";
                                echo $tokenizedComments;
                            ?>
                        </p>
                        <p><strong>Sentiment:</strong>
                            <?php
                                $sentiment = !empty($result['sentiment']) ? esc($result['sentiment']) : "N/A";
                                echo $sentiment;
                            ?>
                        </p>
                    </div>
